...
type ScholarshipStatus = "none" | "partial" | "full"; // Task 8: Union type for scholarship status

type Subject = [string, number]; // Task 4: Tuple [subject: string, marks: number]
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const enrollmentStatusInput = document.getElementById('enrollmentStatusInput') as HTMLSelectElement;
  const subjectInput = document.getElementById('subjectInput') as HTMLInputElement;
  const marksInput = document.getElementById('marksInput') as HTMLInputElement;
  const scholarshipInput = document.getElementById('scholarshipInput') as HTMLSelectElement;
  ...
  function handleAddOrUpdateStudent(name: string = "Anonymous"): void {
    ...
    const scholarship = scholarshipInput.value as ScholarshipStatus;
    ...
    const newSubject: Subject = [subject, marks]; // Task 4: Tuple
    ...
    if (isEditing && editingId !== null) {
      ...
      if (studentIndex !== -1) {
        students[studentIndex] = {
          ...
          gender,
          enrollmentStatus,
          subjects: [...students[studentIndex].subjects, newSubject],
          scholarship,
        };
  ...
  function getAverageMarks(studentId: number): number {
  ...
  function addSubjects(studentId: number, ...newSubjects: Subject[]): void {
  ...
  function findStudentOrThrow(id: number): Student | never {
  ...
  studentTableBody.addEventListener('click', (event: Event) => {
